...
class DBWNode(object):
    def __init__(self):
        ...
        self.controller = Controller(throttle_params, brake_params, wheel_base,
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            if self.time is None:
                ...
                throttle, brake, steering = self.controller.control(sample_time, self.exp_lin_vel,
                                                                    self.act_lin_vel, self.exp_ang_vel)

                vel_err = round((self.exp_lin_vel - self.act_lin_vel), 2)
                # rospy.loginfo('Throttle: {}/{}'.format(round(throttle, 2), vel_err))
                rospy.loginfo('Brake: {}/{}'.format(round(brake, 2), -vel_err))
                # rospy.loginfo('Steering: {}/{}'.format(round(steering, 2), self.exp_ang_vel))
